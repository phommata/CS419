	...
	if event == ord("3"): break
		...
		try:
			...
			for row in results:
				aname = row[0]
				sname = row[1]
				appdate = row[2] # doesnt work  
				apptime = row[3] #doesn't work
				screen.addstr("Appointment with student: " + sanme + " On: " + appdate + " At: " +  apptime + "\n")
		except:
			screen.addstr("No advisor by that name.\n") # parts that don't work cause this to be set off
	elif event == ord("2"):
